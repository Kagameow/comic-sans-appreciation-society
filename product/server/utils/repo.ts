...
import type { Code, GameConfig, Player, Redemption } from '#shared/types/game'
...
export function useRepo() {
  const currentMultiplier = (): number => {
    if (!store.config.multiplierEndsAt)
      return 1
    if (store.config.multiplierEndsAt < Date.now())
      return 1
  ...
  return {
    ...
    redeemMinigameResult(player: Player, codeRef: string, base: number) {
      if (base === 0)
        return { awarded: 0, multiplier: 1, gemUnlocked: false, clueUnlocked: false }
      const multiplier = currentMultiplier()
    ...
    superWinner(): { name: string, at: number } | null {
      if (!store.config.superWinner)
        return null
      const p = this.getPlayerById(store.config.superWinner)
      if (!p || !store.config.superWonAt)
        return null
